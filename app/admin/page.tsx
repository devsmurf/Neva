            ...
            <div className="flex-1">
              ...
              <div className="mt-3 pt-3 border-t border-slate-100">
                <div className="text-xs md:text-sm">
                  <span className="text-slate-500">Bağımlılık:</span>
                  <span className="ml-2 font-medium">
                    {task.dependent_company_id && task.dependent_company?.name
                      ? `🔗 ${task.dependent_company.name}`
                      : '❌ Yok'}
                  </span>
                </div>
              </div>
...
export default function AdminPage() {
  ...
  useEffect(() => {
    console.log('🔐 Admin auth check:', { sessionLoading, user: user?.role })
  ...
  useEffect(() => {
    // Only load data once when user is confirmed admin and data not loaded yet
    if (!user || user.role !== 'admin' || dataLoaded) return

    const loadData = async () => {
      try {
        setLoading(true)
        console.log('🔄 Admin Panel: Initial data load')

        // Load tasks, companies, and projects
        ...
        if (tasksResponse.ok) {
          ...
          console.log('✅ Admin Panel: Loaded', tasksData.length, 'tasks')
          console.log('📋 Pending tasks:', tasksData.filter(t => !t.is_approved).length)
          setTasks(tasksData)
        }
        ...
        setDataLoaded(true) // Mark as loaded to prevent re-execution
      } catch (error) {
    ...
    loadData()
  }, [user?.role, dataLoaded]) // Depend on role and loaded state
  ...
  // Görev reddetme fonksiyonu
  const handleReject = async (id: string) => {
    if (!confirm('Bu görevi reddetmek istediğinizden emin misiniz? Bu işlem geri alınamaz.')) {
      return
    }
  ...
  const handleDeleteTask = async (id: string) => {
    console.log('🗑️ Delete task called with ID:', id)
                    ...
                    return (
                      <tr key={t.id} className="border-b border-slate-100">
                        ...
                        <td className="td">
                          {cAt ? (
                            delta > 0 ? (
                              <span className="inline-flex items-center px-2 py-1 rounded-lg bg-emerald-100 text-emerald-700 text-xs font-medium">{delta} gün erken tamamlandı</span>
                            ) : delta < 0 ? (
                              <span className="inline-flex items-center px-2 py-1 rounded-lg bg-red-100 text-red-700 text-xs font-medium">{Math.abs(delta)} gün geç tamamlandı</span>
                            ) : (
                              <span className="inline-flex items-center px-2 py-1 rounded-lg bg-blue-100 text-blue-700 text-xs font-medium">Gününde tamamlandı</span>
                            )
                          ) : (
        ...
        <div className="card p-6 space-y-6">
          ...
          <div className="bg-slate-50 rounded-lg p-4">
            <div className="text-slate-700 mb-4">
              ...
              <div className="text-xl font-bold">{project?.name || 'Yükleniyor...'}</div>
            </div>
            ...
            <div className="flex items-end gap-3">
              <div>
                ...
                <input
                  ...
                  defaultValue={project?.end_date || '2025-12-29'}
                />
              </div>
